...
class CustomerQuery implements CustomerQueryInterface
{
    ...
    public function findAll(?string $email = null, ?string $name = null, ?int $status = null, int $limit = 20, int $offset = 0): array
    {
        ...
        $params = [];
        if ($email) { $sql .= ' AND c.email LIKE :email'; $params['email'] = "%{$email}%"; }
        if ($name) { $sql .= ' AND (c.name01 LIKE :name OR c.name02 LIKE :name)'; $params['name'] = "%{$name}%"; }
        if ($status) { $sql .= ' AND c.customer_status_id = :status'; $params['status'] = $status; }
    ...
    public function findById(int $id): ?array { return $this->pdo->fetchOne('SELECT * FROM customer WHERE id = :id', ['id' => $id]) ?: null; }
    public function findByEmail(string $email): ?array { return $this->pdo->fetchOne('SELECT * FROM customer WHERE email = :email', ['email' => $email]) ?: null; }
    ...
    public function delete(int $id): void { $this->pdo->perform('DELETE FROM customer WHERE id = :id', ['id' => $id]); }
    public function updatePoint(int $id, int $point): void { $this->pdo->perform('UPDATE customer SET point = point + :point WHERE id = :id', ['id' => $id, 'point' => $point]); }
}
