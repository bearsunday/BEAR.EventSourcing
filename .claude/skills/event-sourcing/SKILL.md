...
Run `vendor/bin/stree <log.json>` (`--full` to expand context keys) or use `Koriym\SemanticLogger\Stree\TreeRenderer`. `examples/semantic-log.json` is the raw `LogJson`; `examples/semantic-log.tree.txt` is its `stree` rendering (committed as a worked example).
