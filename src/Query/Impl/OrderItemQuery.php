...
class OrderItemQuery implements OrderItemQueryInterface
{
    ...
    {
        return $this->pdo->fetchAll('SELECT * FROM order_item WHERE order_id = :order_id', ['order_id' => $orderId]);
    }
    ...
    public function createItems(int $orderId, array $items): void
    {
        ...
        foreach ($items as $item) {
            ...
            $ph = ':' . implode(', :', array_keys($item));
            $this->pdo->perform("INSERT INTO order_item ({$cols}) VALUES ({$ph})", $item);
        }
    }
}
