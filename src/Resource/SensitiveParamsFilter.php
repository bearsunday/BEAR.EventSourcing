 ...
 *   Withholding it leaves the recorded params genuinely insufficient to reproduce the
 ...
 * filtered without flipping replayability even though it also contains `token`.
 *
 ...
 * to stay deterministic, and filtering it would make an otherwise-safe write silently
 * non-replayable. `resetKey`/`authKey`-shaped fields an application actually wants redacted
 * are a `#[Filtered] ParamsFilterInterface` the application binds itself — this package does
 * not know an arbitrary caller's naming conventions well enough to guess safely.
 *
