...
class CartQuery implements CartQueryInterface
{
    ...
    public function findByKeyOrCustomerId(string|null $cartKey, int|null $customerId): array|null
    {
        ...
        $cart['total_price'] = array_reduce($items, static fn ($sum, $item) => bcadd($sum, bcmul($item['price'], (string) $item['quantity'])), '0');
    ...
    public function clear(string|null $cartKey, int|null $customerId): void
    {
        ...
        $this->pdo->perform('DELETE FROM cart_item WHERE cart_id = :cart_id', ['cart_id' => $cart['id']]);
    }
